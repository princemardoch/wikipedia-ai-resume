...
resume_file = Config.files.save_resume
scrapping_failed_file = Config.files.save_scrapping_failed
...
def save_not_valid_url(url, error_cause):
    ...
    with open(scrapping_failed_file, 'a') as file:
...
class Groq_ai:
    @staticmethod
    def resume(instruction, article):
        if article is None:
            return None
...
if __name__ == "__main__":
    ...
    for url in articles_urls:
        ...
        resume_article = Groq_ai.resume(instruction, text_article)
        save_resume(url=url, resume=resume_article)
    print('\n\n FINISH')